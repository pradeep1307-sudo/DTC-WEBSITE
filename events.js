...
const renderEventAgenda = (events) => {
  ...
  let autoScroll;
  const stopAutoScroll = () => window.clearInterval(autoScroll);
  const startAutoScroll = () => {
    stopAutoScroll();
    if (manuallyPaused || window.matchMedia('(prefers-reduced-motion: reduce)').matches) return;
    autoScroll = window.setInterval(() => {
      const halfway = agenda.scrollHeight / 2;
      if (agenda.scrollTop >= halfway - 57) agenda.scrollTo({ top: 0, behavior: 'auto' });
      else agenda.scrollBy({ top: 56, behavior: 'smooth' });
    }, 2600);
  };
...
const renderCalendar = (events) => {
  ...
  const render = () => {
    ...
    for (let day = 1; day <= totalDays; day += 1) {
      ...
      cells += `<div class="event-calendar-day${dateKey === todayKey ? ' is-today' : ''}"><span class="event-calendar-date">${day}</span>${dayEvents.map((event) => `<a href="${eventUrl(event)}">${escapeText(event.title)}${event.time ? `<small>${escapeText(event.time)}</small>` : ''}</a>`).join('')}</div>`;
    }
